...
const EditBlogModal = () => {
  ...
  useEffect(() => {
    if (data.editBlog) {
      setBlogData({
        title: data.editBlog.title,
        content: data.editBlog.content,
        author: data.editBlog.author,
        status: data.editBlog.status,
        tags: data.editBlog.tags.join(", "),
        featuredImage: null,
      });
    }
  }, [data.editBlog]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${apiURL}/api/blogs/${data.editBlog._id}`,
        formData,
        {
          headers: {
      ...
      dispatch({ type: "updateBlog", payload: response.data });
      dispatch({ type: "editBlogModal", payload: false });
      dispatch({ type: "editBlog", payload: null });
    } catch (err) {
              ...
              <form onSubmit={handleSubmit}>
                <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  ...
                  <div className="mb-4">
                    ...
                    {data.editBlog.featuredImage && (
                      <div className="mb-2">
                        <img
                          src={`${apiURL}/uploads/blogs/${data.editBlog.featuredImage}`}
